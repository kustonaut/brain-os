...
- **Day of week** — determines urgency (Mon = fresh week, Thu = prep, Fri = EOW wrap)
- **Time of day** (if available) — Morning (before 12 PM): brief/planning mode. Afternoon (12–5 PM): execution/action mode. Evening (after 5 PM): review/EOD mode.
- **Week context:** Check if today falls within a 🔴 THIS WEEK window in Action_Items.md and flag any items due today/tomorrow.

Use this temporal context to:
1. Prioritize recommendations (urgent items first on Mon–Wed, wrap-up items on Thu–Fri)
...
## Context
This workspace is **Brain** — {{your_name}}'s ({{your_role}}, {{your_team}}) daily operating system. Tracks {{number}} charter areas: {{list_your_charters}}. On track, on and off work (except personal).

## Folder Convention
Each charter folder follows a flat structure:
- **Active deliverables at root** — current docs, reports, decks live at the folder root
- **`Reference/`** — old transition docs, background material, partner deep-dives
- **`Recordings/`** — .mp4 meeting recordings
- **`Transcripts/`** — meeting transcript text files
...
- `Goals.md` — **OKRs, key results, metric targets, weekly reflection** (gravity center — every skill reads this)
- `Charter_Areas.md` — Scope, features, architecture notes
- `Key_Contacts.md` — People, aliases, roles
- `Partners.md` — External partner details
- `Action_Items.md` — Open tasks, action items, recurring meetings
- `Processes_Reference.md` — Incident process, release flow, architecture decisions, technical notes
- `AI_PM_Skills.md` — AI PM tech stack, prompt technique ladder, temperature table, model routing
- `Agent_Build_Checklist.md` — 10-question pre-mortem checklist (run before building any automation)
- `Agent_Failure_Playbook.md` — Dev/LLM/Production failure taxonomy + recovery procedures
- `Trust_Boundaries.md` — What each script/skill is allowed to do; data classification; escalation triggers
...
- `emails_YYYY-MM-DD.md` — Outlook email highlights (last 24h, keyword-filtered)
- `ado_YYYY-MM-DD.md` — ADO work item states, assigned items
- `calendar_YYYY-MM-DD.md` — Today/tomorrow meetings, free slots, conflicts
- `chats_YYYY-MM-DD.md` — Teams chat summaries (if Graph API available)
...
## Landing Zone
- **`_Inbox/`** — Drop any new files here. Automation parses and routes them to correct charter folders daily.
- Use subfolder names to force routing: `_Inbox/project-alpha/`, `_Inbox/project-beta/`, etc.

## Automation System
- **`_Automation/`** — Daily orchestrator + pipeline scripts
...
- **Pipeline steps:** cleanup → parse_inbox → fetch_emails → fetch_chats → fetch_ado → fetch_calendar → generate_brief → block_calendar → sync_instructions
- **Prompt templates:** `_Automation/prompt_templates/` — versioned prompt files with `{{variable}}` placeholders
- **Cleanup:** `_Automation/cleanup.ps1` — archives signals >14d, briefs >30d, deletes logs >30d
- **Weekly snapshot:** `_Automation/weekly_snapshot.ps1` — Friday aggregation of week's signals
- **Portal:** `_Automation/serve_artifacts.py` — Brain at http://localhost:8765

## Automation Rules
These apply whenever building or modifying automation, scripts, or prompts:

1. **Temperature-first**: Before writing any prompt, check `config.json → ai_guidance.temperature_by_task`. Use 0.1 for data/brief tasks, 0.5 for comms, 0.8 for creative.
...
- **What:** Interactive daily close-out — reviews tasks, captures wins, updates Goals.md, writes daily log.
...
- **What:** Manage your PM intelligence workspace — setup wizard, portal launch, pipeline, health check.
...
- **What:** Advanced Azure DevOps reporting — sprint health, epic burndown, stale item detection.
...
- **What:** Draft Teams messages — standups, channel announcements, escalations.
...
1. **Always open newly created artifacts** — After generating any file (.pptx, .docx, .xlsx, .pdf, .html), immediately open it using `Start-Process` so the user can review it.
2. **Always use consistent theme** — When creating Office documents, use your org's brand colors. Default: Segoe UI fonts, Blue (#0078D4) accents, Dark Navy (#1B2A4A) headers, Body Gray (#404040) text.
3. **Always save to temp first** — When using COM automation, save to `$env:TEMP` first, then copy to the workspace path to avoid file-lock issues.
4. **Always confirm artifact paths** — After creating a file, print the full path so the user knows where it landed.
5. **Always offer proactive next steps** — After completing any task, suggest related follow-up actions.
6. **Always check Goals.md first** — Before recommending priorities or triaging work, read `Goals.md` to align with current OKRs.
7. **Always update Knowledge Base on new info** — When the user shares raw information (wiki pages, emails, docs, meeting notes), ALWAYS: (a) save to correct charter folder, (b) extract contacts → `Key_Contacts.md`, (c) extract processes → `Processes_Reference.md`, (d) extract action items → `Action_Items.md`.
...
Never mark a task complete without proving it works — run the script, check the file, open the artifact.
...
When encountering a broken state: just fix it. Check logs, errors, file states — then resolve.
